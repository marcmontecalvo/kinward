...
import { LayoutGrid, LayoutItem } from "./surfaces/layoutStyleAdapter";
...
export function App() {
  ...
  return (
    <main className={`shell surface-shell surface-${surface}`} data-surface={surface} data-privacy={context.privacy} data-interaction={`${context.touch ? "touch" : "no-touch"}-${context.keyboard ? "keyboard" : "no-keyboard"}`} data-viewing-distance={context.viewingDistance} data-layout-id={resolvedLayout.layout.id} data-layout-version={resolvedLayout.layoutVersion} data-context-version={resolvedLayout.contextVersion} data-layout-provenance={resolvedLayout.provenance} data-fallback-reason={resolvedLayout.fallbackReason ?? "none"}>
      ...
      <LayoutGrid className="grid resolved-grid" aria-label="Synthetic assistant overview" columns={resolvedLayout.layout.grid.columns} gapPx={resolvedLayout.layout.grid.gapPx}>
        {resolvedLayout.instances.map((instance) => {
          ...
          return (
            <LayoutItem id={instance.id} className="layout-instance" key={instance.id} data-instance-id={instance.id} column={instance.column} row={instance.row} columns={instance.columns} rows={instance.rows}>
              {resolved.ok ? (() => { const Renderer = resolved.definition.render; return <Renderer cardId={instance.id} title={instance.title} view={resolved.view} onIntent={handleIntent} />; })() : <SafeCardFallback title={instance.title} reason={resolved.reason} />}
            </LayoutItem>
          );
        })}
      </LayoutGrid>
